...
void get_imei_durum(char *buf, size_t sz) {
    char cur[64] = {0}, bak[64] = {0};
    FILE *f = fopen("/dev/imeidata/efs_current/imei.txt", "r");
    if (f) { fgets(cur, sizeof(cur), f); char *nl = strchr(cur, '\n'); if (nl) *nl = 0; fclose(f); }
    f = fopen("/dev/imeidata/efs_backup/imei.txt", "r");
    if (f) { fgets(bak, sizeof(bak), f); char *nl = strchr(bak, '\n'); if (nl) *nl = 0; fclose(f); }
    if (cur[0] && bak[0] && strcmp(cur, bak) == 0)
        snprintf(buf, sz, "%s (orijinal)", cur);
    else if (cur[0])
        snprintf(buf, sz, "%s (değişmiş!)", cur);
    else
        snprintf(buf, sz, "EFS bulunamadı");
}
...
int main() {
    char kernel[128], uptime[64], cpu[128], ram[64], ip[64], devid[128], imei[128];
    ...
    get_imei_durum(imei, sizeof(imei));
