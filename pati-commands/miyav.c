...
int init_socket() {
    ...
    if (s < 0) {
        perror("socket");
        fprintf(stderr, "Hey kanka, galiba yönetici yetkisini unuttun. Onsuz ben çalışamam ya!\n");
        return -1;
    }
...
int send_packet(int sock, char *sendbuf, struct sockaddr_in *dest) {
    ...
    if (bytes < 0)
        perror("sendto fonksiyonu başarısız..");
    return bytes;
}
...
void process_reply(char *recvbuf, int bytes, struct sockaddr_in *from, int seq,
    ...
    if (icmp_reply->icmp_type == ICMP_ECHOREPLY &&
        ...
        printf("%d bayt, %s adresinden: icmp_seq=%d ttl=%d süre=%.2f ms\n",
               bytes - hlen,
...
void cleanup(int sig) {
    ...
    printf("%d paket gönderildi, %d paket alındı, %d%% kayıp\n",
           tx_count, rx_count,
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Kullanım: %s <hedef>\n", argv[0]);
        return 1;
    }

    struct hostent *host = gethostbyname(argv[1]);
    if (!host) {
        fprintf(stderr, "Sunucu veya Adres bulunamadı: %s\n", argv[1]);
        return 1;
    }
	    ...
	    gettimeofday(&tv_start, NULL);
            ...
            if (bytes < 0) {
                if (errno == EAGAIN || errno == EWOULDBLOCK)
                    printf("Zaman aşımı: icmp_seq=%d\n", i);
                else
                    perror("recvfrom");
            } else {
                process_reply(recvbuf, bytes, &from, i, &tv_start, &tv_end);  // tx_count-1 yerine i
            }
